...
<details>
<summary><strong>오픈런 게시글 조회 : 동시성 제어 성능 개선</strong></summary>

## 프로젝트 수행 경과 - 트러블슈팅

# 오픈런 게시글 조회 : 동시성 제어 성능 개선

---

### 초기 구현

1. Redis ZSet을 통한 순위 구현  
2. 게시글의 제한 인원과 ZSet의 크기를 비교하여 접근 제한  
3. 동시 요청 허용 → 순위 재검증 & ZSet에서 삭제 후 예외처리  
...
- 재검증 방식은 스레드 증가에 매우 취약 (**평균 응답 시간 약 4배 증가**)  
- Lettuce 방식은 스레드 증가에도 성능을 유지하지만, 오류 존재  
- ⇒ **Redisson 적용**  
