...
* **기술적 대응**
    ...
    * <strong>Redis 기반 캐싱:</strong> 특정 시간에 자동 공개되는 게시글에 대해 <strong>조회 부하 분산 및 접근 타이밍 동기화</strong>
...
오픈런 게시글은 특정 시간에만 공개되며, 그 중에서도 선착순 게시글은 제한된 인원만 접근 가능한 게시글입니다.

초기 구현은 Redis의 ZSet을 사용해 요청 순서를 기록하고, 게시글 접근 시 ZSet의 크기와 해당 게시글의 제한 인원을 비교하여 초과 접근을 차단하는 방식이었습니다.

요청을 먼저 받아들이고, 이후 ZSet에서 초과된 유저를 제거하는 방식이었기 때문에, 트래픽이 증가할수록 검증 과정에 병목이 발생하여 성능이 저하되는 문제가 있었습니다.


### 성능 측정 결과

| 동시성 제어 방식 | 에러율 | 평균 응답 시간(ms) | TPS   |
|-------------------|---------|----------------------|---------|
| 재검증 방식       | 0%      | 5522.3               | 42.3    |
| Lettuce           | 0.17%   | 9282.8               | 36.6    |
| Redisson          | 0%      | 9411.2               | 36.6    |

→ 요청 스레드 수 증가 시 성능 비교  

| 동시성 제어 방식 | 에러율 | 평균 응답 시간(ms) | TPS   |
|-------------------|---------|----------------------|---------|
| 재검증 방식       | 0%      | 21143.3              | 36.1    |
| Lettuce           | 0.05%   | 15465.2              | 40.7    |
| Redisson          | 0%      | 15075.0              | 41.8    |


### 기존 성능 분석

- 현재 ZSet 크기 검증과 사용자 정보를 ZSet에 삽입하는 과정 사이에서 **순간적으로 초과 삽입 허용**
- 사후 예외처리 방식은 반복적인 ZSet 접근 유발
- 스레드 수가 3000 → 4000 증가함에 따라 평균 응답 시간은 **약 4배 증가**, RPS는 **15% 하락**
    
    즉, 단순 조건문 기반 로직은 **요청 폭주 상황에서의 성능에 한계 존재**

### 개선 전략

초과 삽입 자체를 사전에 차단하기 위해, 요청 처리 이전에 임계 구역을 보호하는 락 기반 제어 방식으로 개선했습니다.

1. **Lettuce 기반 락 적용**
    - Redis의 `SETNX`(Set if Not Exists) 명령어를 이용해 락을 획득한 후에만 ZSet 삽입 처리
    - RPS는 40.7로 개선되었으나, 일부 요청이 락 획득에 실패해 에러율(0.05~0.17%) 발생
2. **Redisson 기반 분산 락 적용**
    - pub/sub 기반 분산락으로 안정적인 락 획득 및 자동 만료 기능 제공
    - 성능이 안정적으로 유지되었고, 에러율은 0%로 문제 없이 처리
  

### 결과
- **Redisson** 적용으로 RPS 손실 없이 **응답 속도 약 30% 개선**, 에러율은 **0% 유지**
...
단순 조건 검증 방식은 스레드 수 증가에 매우 취약하다는 점을 실측 데이터를 통해 명확히 확인할 수 있었습니다.

Lettuce는 동시성 문제를 해결하는 데 유효했지만, 락 획득 실패로 인해 소량의 오류가 발생하였고, 이에 대한 별도의 예외 처리 로직이 필요했습니다.
